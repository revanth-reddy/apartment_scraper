...
def get_images(soup, fields):
    ...
    # find li with class paidImageLarge
    obj = soup.find('li', class_='item paidImageLarge')
    if obj is not None:
...
def get_description(soup, fields):
    ...
    if obj is not None:
        obj = obj.find_all('p')
        if obj is not None:
            fields['description'] = prettify_text(obj[0].getText())
...
def get_property_size(soup, fields):
    ...
    obj = soup.find('div', class_='sqftColumn column')
    if obj is not None:
        data = obj.findAll('span')[1].text
        data = prettify_text(data)
        fields['size'] = str(data)
...
def get_fees(soup, fields):
    ...
    if obj is not None:
        for expense in obj.find_all('div', class_='descriptionWrapper'):
            description = expense.find('span', class_='expense-description').getText()
            # description = prettify_text(description)

            price = expense.find('span', class_='expense-cost').getText()
...
def get_property_address(soup, fields):
    ...
    # Fetch the name from here
    fields['name'] = find_addr(script,'listingName')
...
def main():
    ...
    config_file = os.path.join(os.path.dirname(__file__), "config_example.ini")
